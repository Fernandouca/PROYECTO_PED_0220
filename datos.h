...
void datosClientes(cliente<producto> &cliente1){//solo recolecta los datos de los clientes
    system("cls");
    fflush(stdin);
...
bool datosAdmin(){//agarra los datos que se ingresen y los compara
...
void mostrarProducto(nodo<producto> *lista,int opcion){
    ...
    switch(opcion){
        case 1:
            while((actual!=NULL) && (actual->dato.codigo>=1001) && (actual->dato.codigo<=1999)){//verifica que no este fuera de la categoria ni al final de lista
