...
void agglista(nodo<T> *&lista,T aggdato){
    ...
    while((aux1!= nullptr) && (aux1->dato.codigo < aggdato.codigo)){//comprueba que A).la lista no este vacia y B). que en la lista de codigos no podas meter un producto en la categoria que no es
...
nodo<T>* buscarint(nodo<T> *lista,int buscador){
    ...
    nodo<T> *actual = new nodo<T>();//el nodo para buscar
    nodo<T> *aux1 = new nodo<T>();//el nodo que me ayuda despues

    actual = lista;//pongo el nodo al iicio de la lista


    while((actual != nullptr) && (actual->dato.codigo <= buscador)){//comprueba que no este en el final de la lista y que el paramentro de busqueda no se vaya adelante de un valor mayor al qu busco
        if(actual->dato.codigo == buscador){//compara si los datos coinciden
